...
local JSON
local tokennum = assert(tonumber(select(1, ...)), "Invalid token amount")
assert(tokennum > 0, "Invalid token amount")
...
local c = io.read("*a")
local n = JSON:decode(c)
local idx
math.randomseed(os.time() + os.clock() * 1000 + #c)

local function floatCmp(a, b)
...
while tokennum > 0 do
	...
	repeat
		...
		local prev = n[idx - 1]
		local next = n[idx + 1]
		local is_simul = (prev and prev.effect == 1 and floatCmp(prev.timing_sec, target.timing_sec))
		is_simul = is_simul or (next and next.effect == 1 and floatCmp(next.timing_sec, target.timing_sec))
	until not(is_simul) and target.effect == 1
	
	target.effect = 2
	tokennum = tokennum - 1
end

io.write(JSON:encode(n))
os.exit(0)
